...
# "no SSO provider returned from management". Offer extra fallback ports BELOW 49152 so the DEFAULT
# dynamic range can never take them (the client picks the first free one). Caveat: "below 49152" only
# dodges the default range. The client skips ANY port in Windows' excluded-port set, which also holds
# explicit administrator reservations (netsh int ipv4 add excludedportrange) at ANY port number - so
# these four ports must NEVER be reserved on a client machine, or the client skips them and login
# re-breaks in the hardest-to-diagnose way. These four ports must also be registered as loopback
# redirect URIs on the Entra app (Entra side owned by Mike). COM-188.
